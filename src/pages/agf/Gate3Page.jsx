...
export default function Gate3Page({ projectId }) {
  ...
  if (!project) {
    return (
      <EmptyState title="Projeto não encontrado" description="O projectId informado não existe nos dados mockados. Volte ao portfólio e selecione um projeto válido.">
  ...
  const data = {
    client: mockClients.find((item) => item.id === project.clientId),
    team: filterByProject(mockProjectTeam, projectId),
    workstreams: filterByProject(mockProjectWorkstreams, projectId),
    activities: filterByProject(mockGate3Activities, projectId),
    pendingItems: filterByProject(mockGate3PendingItems, projectId),
    risks: filterByProject(mockGate3Risks, projectId),
    gaps: filterByProject(mockGate3Gaps, projectId),
    decisions: filterByProject(mockGate3Decisions, projectId),
    issues: filterByProject(mockGate3Issues, projectId),
    changeRequests: filterByProject(mockGate3ChangeRequests, projectId),
    deliverables: filterByProject(mockGate3Deliverables, projectId),
    dependencies: filterByProject(mockGate3Dependencies, projectId),
    statusReports: filterByProject(mockGate3StatusReports, projectId),
    actionPlans: filterByProject(mockGate3ActionPlans, projectId),
    meetings: filterByProject(mockGate3Meetings, projectId),
  };
  ...
  return (
    <div className="space-y-6">
      <PageTitle
        title={`Gate 3 — ${project.name}`}
        subtitle="Construção, validações, integrações, gestão diária da implantação e controle operacional do projeto selecionado."
      />
      <section className="rounded-2xl bg-slate-900 p-6 text-white shadow-lg">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-3 max-w-3xl text-sm leading-relaxed text-slate-300">
              Gate 3 — Execução Controlada consolida atividades, riscos, pendências, gaps, decisões, mudanças, dependências, entregáveis e status semanal deste projeto.
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            <StatusBadge variant="info">{gateStatus}</StatusBadge>
            <StatusBadge variant="neutro">{data.team.length} integrantes</StatusBadge>
          </div>
        </div>
        <div className="mt-6 flex flex-wrap gap-3">
          ...
          <button onClick={() => navigateTo(`/projetos/${project.id}/agf`)} className="rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-700">Voltar à Jornada AGF</button>
        </div>
      </section>
      <ProjectContextHeader project={project} healthStatus={health.status} />
      <Gate3KpiCards project={project} data={data} />
      <Gate3StepsPanel steps={gate3Steps} />
      <Gate3Tabs project={project} workstreams={data.workstreams} data={data} health={health} />
      <Gate3AcceptanceCriteria />
      <Gate3HealthPanel health={health} data={data} />
    </div>
  );
}
